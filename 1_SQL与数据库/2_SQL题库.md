...
- **第一范式 (1NF)**：确保每列都是原子的（不可再分）。例如，不能有一个 “phones” 列存放多个电话号码，应该拆分成多行或另一个表。
- **第二范式 (2NF)**：在满足 1NF 的基础上，要求所有非主键列必须**完全依赖**于整个主键（针对复合主键的情况）。
- **第三范式 (3NF)**：在满足 2NF 的基础上，要求所有非主键列之间**不能有传递依赖**，即非主键列必须直接依赖于主键。
...
在 `LEFT JOIN` 中，**左表永远是驱动表**。因为它的语义就是“要返回左表的全部行”。
优化器没有选择权，它必须从左表开始，然后去右表（被驱动表）中为每一行寻找匹配。因此，**在 `LEFT JOIN` 中，右表的连接字段上建立索引尤为重要**
...
所有常见的聚合函数（`SUM`, `AVG`, `COUNT`, `MAX`, `MIN`）都可以作为窗口函数使用，实现**累计计算**。
