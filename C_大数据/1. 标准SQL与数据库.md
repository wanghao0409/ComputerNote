...
### Merge INTO
  ...
  `MERGE INTO`（常被称为 **UPSERT** 语句，即 **UPDATE + INSERT** 的合并）是 SQL 标准中用于**根据源表数据对目标表执行插入、更新或删除操作**的语句。它允许你通过一条语句将源表与目标表进行比较，并根据匹配情况执行不同的数据操作，非常适合数据同步、缓慢变化维（SCD）处理等场景。
...
[ WHEN NOT MATCHED BY SOURCE THEN DELETE ];  -- 可选，处理目标表多出的行
...
`EXISTS` 子查询一旦找到满足条件的行，就会立即停止扫描（短路评估），因此通常比 `COUNT(*) > 0` 等方式更高效
